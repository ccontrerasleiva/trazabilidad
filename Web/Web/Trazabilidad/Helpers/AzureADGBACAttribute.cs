namespace Trazabilidad.Helpers
{
    ...
    public class AzureADGBACAttribute : AuthorizeAttribute
    {
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            //Operación
            if (identity.IsInGroup(AzureGroups.PlatformOperacion))
                return;

            //Externo
            if (identity.IsInGroup(AzureGroups.PlatformExterno))
                return;

            //RRHH
            //if (identity.IsInGroup(AzureGroups) && !controller.ToLower().Equals("users"))
